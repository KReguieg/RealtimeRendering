...
public:
    ...
    // construct with name and from Qt array data
    VertexBuffer(QVector<T> data,
...
template<typename T>
VertexBuffer<T>::VertexBuffer(QVector<T> data,
...

{
    ...
    buffer_.allocate(data.constData(), int(data.size() * sizeof(T)));
    buffer_.release();
